...
class SettingsManager {
  ...
  async executeOnAllTabs() {
    ...
    if (executeAllButton) {
      ...
      executeAllButton.textContent = '执行中...';
    }
    
    try {
      chrome.runtime.sendMessage({ action: 'log', message: 'Popup开始执行所有页面的快捷操作' });
      
      const result = await new Promise((resolve) => {
        chrome.runtime.sendMessage({ action: 'executeOnAllTabs' }, (response) => {
          resolve(response);
        });
      });
      ...
      if (result.success) {
        ...
        this.showStatus(result.message || '执行失败，请重试', 'error');
      }
      
    } catch (error) {
      chrome.runtime.sendMessage({ action: 'log', message: `Popup执行出错: ${error.message}` });
      console.error('执行快捷操作时出错:', error);
      this.showStatus(error.message || '执行失败，请重试', 'error');
    } finally {
      if (executeAllButton) {
        executeAllButton.disabled = false;
        executeAllButton.textContent = '执行所有页面';
      }
    }
  }
